...
public class ParticleFiltering {
	...
	@Deprecated
	public double[] particleFilteringProb(AssignmentProposition[] e) {
		// local variables: W, a vector of weights of size N
		double[] W = new double[N];
		...
		for(AssignmentProposition[] propArr : S_tp1){
			for(AssignmentProposition prop : propArr){
				System.out.println(prop);
			}
		}
		/* step 3 */
		// S <- WEIGHTED-SAMPLE-WITH-REPLACEMENT(N, S, W)
		return Util.normalize(W);
	}
